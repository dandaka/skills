...
- Telethon SQLite session allows only one connection at a time. If you get `database is locked`, wait for other `tg.py` processes to finish.
- `--limit 10000` effectively means "all messages" for most chats.
- Re-running overwrites `history.json` with a fresh full export.
